...
- `release-weekly.yml`: manual `cut` with an optional full main SHA. If a release branch exists, it leaves that train in QA. Otherwise it selects the current main head and requires its configured checks to have succeeded; it never silently picks an older commit. Release-branch pushes or manual `prepare` run reusable lint/Go checks, publish one candidate OTA branch, build one DOCR image, and build both store binaries only when native inputs changed. The final job deploys staging, saves the manifest and native artifacts to a draft GitHub release, and requests production approval.
- `release-promote.yml`: presents the exact candidate for one GitHub `production` environment approval. The publishing job itself is gated; it rechecks branch head, the current candidate pointer, and configuration, then promotes the recorded artifacts and publishes the GitHub release. It deletes the release branch on success. QA and production use separate concurrency groups so an approval wait cannot block QA fixes. There are no separate approval receipts or resume operations.
- `release-build-ios.yml` / `release-build-android.yml`: reusable native QA builders the train calls, also manually dispatchable for a dedicated `release-qa` installation. EAS builds locally on GitHub runners, allocates remote build numbers, and submits to TestFlight / Play internal testing. The workflows record actual binary identity and checksums; production does not rebuild. The manual `build-submit-ios.yml` / `build-submit-android.yml` dispatch workflows remain for one-off store builds outside the train.
...
3. Merge QA fix PRs into the release branch. Each push automatically prepares a replacement candidate; previous approval requests fail their head/pointer checks. Forward-port each QA fix to main as a manual cherry-pick, in merge order, through normal CI/review.
...
Complete configuration and a supervised rehearsal before cutting the first train:
...
- `RELEASE_BOT_TOKEN`: fine-grained repository bot token able to create release branches/releases and read CI checks/environment settings. It must trigger workflows when pushing branches. Configure Actions permissions to allow the final QA job to dispatch `release-promote.yml` using its GitHub token.
...
Before production starts, re-run all QA jobs or dispatch `prepare` to create a fresh candidate, then repeat affected QA and approve that candidate. Cancel obsolete pending approval runs. Cherry-pick each QA fix to main manually, in merge order, before dependent fixes. To abandon a train, cancel its runs and delete its release branch/draft manually.
...
For OTA/web rollback, cancel active release runs, choose the previous published manifest, manually repoint the production OTA channel to its recorded branch ID and restore each web target to its recorded DOCR digest. Verify the served OTA update identity, web `/_release` and health, and confirm native runtime compatibility. Store rollout intervention or a new native build is manual. There is no automatic rollback or recovery controller.
